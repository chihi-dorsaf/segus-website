...
class ProjectViewSet(viewsets.ModelViewSet):
    ...
    def employee_dashboard(self, request):
        ...
        try:
            ...
            try:
                for task in urgent_tasks:
                    task_data = {
                        'id': task.id,
                        'title': task.title,
                        'priority': getattr(task, 'priority', 'medium').lower() if hasattr(task, 'priority') else 'medium',
            ...
            performance_level = 'Excellent' if performance_percentage >= 90 else \
                              'Bon' if performance_percentage >= 70 else \
                              'Moyen' if performance_percentage >= 50 else 'À améliorer'
