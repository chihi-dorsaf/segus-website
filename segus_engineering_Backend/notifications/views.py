...
class ProjectAssignment(APIView):
    ...
    def post(self, request):
        ...
        if employee_ids:
            ...
            emp_user_ids = Employee.objects.filter(id__in=employee_ids).values_list('user_id', flat=True)
            users += list(User.objects.filter(id__in=emp_user_ids))
        if employee_matricules:
            emp_user_ids = Employee.objects.filter(matricule__in=employee_matricules).values_list('user_id', flat=True)
            users += list(User.objects.filter(id__in=emp_user_ids))
