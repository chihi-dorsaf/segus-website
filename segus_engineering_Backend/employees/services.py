...
def send_welcome_email(employee, password):
    ...
    try:
        ...
        context = {
            ...
            'password': password,
            'login_url': f"{settings.FRONTEND_URL}/login" if hasattr(settings, 'FRONTEND_URL') else "http://localhost:4200/login"
        }
        ...
        logger.error(f"[EmployeeService] Erreur lors de l'envoi de l'email de bienvenue a {employee.email}: {str(e)}")
        return False
...
def send_password_reset_email(employee, new_password):
    ...
    try:
        ...
        logger.info(f"[EmployeeService] Email de reinitialisation envoye avec succes a {employee.email}")
        return True
        
    except Exception as e:
        logger.error(f"[EmployeeService] Erreur lors de l'envoi de l'email de reinitialisation a {employee.email}: {str(e)}")
        return False
...
def send_forgot_password_email(email, reset_token):
    ...
    """
    try:
        reset_url = f"{settings.FRONTEND_URL}/reset-password?token={reset_token}" if hasattr(settings, 'FRONTEND_URL') else f"http://localhost:4200/reset-password?token={reset_token}"
        ...
        logger.error(f"[EmployeeService] Erreur lors de l'envoi de l'email de mot de passe oublie a {email}: {str(e)}")
        return False 