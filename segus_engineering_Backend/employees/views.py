...
from .serializers import (
    ...
    EmployeeExportSerializer, EmployeeImportSerializer,
    WorkSessionSerializer, WorkSessionCreateSerializer, EmployeeWorkStatsSerializer,
    WorkSessionDetailSerializer, EmployeeWorkHistorySerializer, 
    DailyWorkStatsSerializer, WeeklyWorkStatsSerializer, MonthlyWorkStatsSerializer,
...
class EmployeeViewSet(viewsets.ModelViewSet):
    ...
    search_fields = ['matricule', 'user__username', 'user__email', 'user__first_name', 'user__last_name', 'position']
    ...
    def list(self, request, *args, **kwargs):
        ...
        try:
            ...
            if page is not None:
                ...
                logger.info(f"[EmployeeViewSet] {len(page)} employes recuperes (pagines)")
                return self.get_paginated_response(serializer.data)

            serializer = self.get_serializer(queryset, many=True)
            logger.info(f"[EmployeeViewSet] {queryset.count()} employes recuperes")
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            with transaction.atomic():
                ...
                logger.info(f"[EmployeeViewSet] Employe cree: {employee.full_name} - {employee.matricule}")
    ...
    def update(self, request, *args, **kwargs):
        ...
        try:
            ...
            logger.info(f"[EmployeeViewSet] Employe {updated_employee.full_name} mis a jour avec succes")
    ...
    def employee_stats(self, request):
        ...
        try:
            ...
            employees_by_position = Employee.objects.values('position').annotate(
                count=Count('id')
            ).order_by('-count')
            position_stats = {}
            ...
            logger.info(f"[EmployeeViewSet] Statistiques calculees: {total_employees} employes total")
    ...
    def export(self, request):
        ...
        try:
            ...
            # En-têtes
            headers = [
                'ID Employé', 'Nom Complet', 'Email', 'Position',
                'Date d\'Embauche', 'Salaire', 'Statut', 'Date de Création'
            ]
            ...
            logger.info(f"[EmployeeViewSet] Export CSV reussi: {employees.count()} employes exportes")
            return response
    ...
    def import_employees(self, request):
        ...
        try:
                ...
                try:
                    ...
                    with transaction.atomic():
                        ...
                        if User.objects.filter(username=row['username']).exists():
                            errors.append(f"Ligne {row_num}: L'utilisateur '{row['username']}' existe déjà")
                            continue
            ...
            logger.info(f"[EmployeeViewSet] Import termine: {imported_count} employes importes, {len(errors)} erreurs")
    ...
    def search(self, request):
        ...
        try:
            ...
            logger.info(f"[EmployeeViewSet] Recherche terminee: {queryset.count()} resultats trouves")
    ...
    def toggle_status(self, request, pk=None):
        ...
        try:
            ...
            logger.info("[EmployeeViewSet] Status changed for %s to %s", employee.full_name, new_status)
    ...
    def reset_password(self, request, pk=None):
        ...
        try:
            ...
            if email_sent:
                logger.info("[EmployeeViewSet] Password reset for %s, email sent", employee.full_name)
                return Response({
                    'message': f'Mot de passe réinitialisé avec succès. Un email a été envoyé à {employee.email}',
                    'email_sent': True
                })
            else:
                logger.warning("[EmployeeViewSet] Password reset for %s, but email failed", employee.full_name)
                return Response({
                    'message': f'Mot de passe réinitialisé avec succès, mais l\'envoi de l\'email a échoué.',
                    'email_sent': False,
    ...
    def forgot_password(self, request):
        ...
        try:
            ...
            try:
                ...
                logger.info(f"[EmployeeViewSet] Demande de reset password pour email inexistant: {email}")
                return Response({
        ...
        try:
            ...
            urls_info = {
                ...
                'available_actions': [
                    ...
                    'employee_stats', 'export', 'import_employees', 'generate_employee_id',
    ...
    def current_session(self, request):
        ...
        try:
            ...
            employee, created = Employee.objects.get_or_create(
                user=request.user,
                defaults={
                    'matricule': f'EMP{request.user.id:04d}',
                    'is_active': True
                }
            )
            
            if created:
                logger.info(f"[WorkSessionViewSet] Profil employé créé automatiquement pour user {request.user.id}")
            ...
            if current_session:
                ...
                return Response({'message': 'Aucune session active'})
        except Exception as e:
    ...
    def employee_stats(self, request):
        ...
        try:
            ...
            for employee in employees:
                ...
                if (employee_stats['total_hours_today'] > 0 or 
                    employee_stats['total_hours_week'] > 0 or 
                    employee_stats['current_session_status'] in ['active', 'paused']):
...
class EmployeeWorkStatsViewSet(viewsets.ViewSet):
    ...
    def export_sessions(self, request):
        ...
        try:
            ...
            writer = csv.writer(response)
            writer.writerow([
                'ID', 'Employé', 'Matricule', 'Date Début', 'Date Fin', 
                'Durée (h)', 'Pause (h)', 'Statut', 'Notes'
            ])
            
            for session in sessions:
                duration = session.duration_hours if hasattr(session, 'duration_hours') else 0
                pause_time = session.total_pause_time.total_seconds() / 3600 if session.total_pause_time else 0
    ...
    def admin_dashboard(self, request):
        ...
        employees_at_work = WorkSession.objects.filter(
            status='active'
        ).values('employee').distinct().count()
        
        # Employés en pause
        employees_on_break = WorkSession.objects.filter(
            status='paused'
        ).values('employee').distinct().count()
    ...
    def _get_employees_on_break(self):
        ...
        paused_sessions = WorkSession.objects.filter(
            status='paused',
            employee__user__role='EMPLOYE'
        ).select_related('employee')
        
        employees_on_break = []
        for session in paused_sessions:
            pause_duration = timezone.now() - session.pause_start_time if session.pause_start_time else timezone.now() - session.start_time
    ...
    def _calculate_employee_work_history(self, employee):
        ...
        for session in recent_sessions:
            ...
            if session.total_pause_time:
                session_data['pause_duration_formatted'] = str(session.total_pause_time).split('.')[0]
            else:
