...
from gamification.models import (
    DailyObjective, SubTask, DailyPerformance, 
    MonthlyPerformance, Badge, EmployeeBadge
)
...
class DailyPerformanceModelTest(TestCase):
    ...
    def test_create_daily_performance(self):
        ...
        self.assertEqual(performance.work_hours, Decimal('7.50'))
    ...
    def test_calculate_performance_goals_met(self):
        ...
        performance = DailyPerformance.objects.create(
            employee=self.employee,
            date=date.today(),
            work_hours=Decimal('10.00')  # 2h d'overtime
        )
        ...
        self.assertEqual(performance.stars_earned, Decimal('0.25'))  # ¼ étoile
        self.assertGreater(performance.points_earned, 0)
    ...
    def test_calculate_performance_goals_not_met(self):
        ...
        performance = DailyPerformance.objects.create(
            employee=self.employee,
            date=date.today(),
            work_hours=Decimal('6.00')  # Moins que l'objectif de 8h
        )
        ...
        self.assertEqual(performance.overtime_hours, Decimal('0.00'))
        self.assertEqual(performance.stars_earned, Decimal('0.00'))  # Pas d'étoile
        self.assertEqual(performance.points_earned, 5)  # 1 point par sous-tâche
...
class MonthlyPerformanceModelTest(TestCase):
    ...
    def test_create_monthly_performance(self):
        ...
        performance = MonthlyPerformance.objects.create(
            ...
            total_subtasks=200,
            total_work_hours=Decimal('160.00'),
            total_overtime_hours=Decimal('40.00')
        )
        
        self.assertEqual(performance.employee, self.employee)
        self.assertEqual(performance.total_subtasks, 200)
    ...
    def test_calculate_monthly_performance_with_bonus(self):
        ...
        for day in range(1, 21):  # 20 jours de travail
            ...
            DailyPerformance.objects.create(
                ...
                work_hours=Decimal('10.00'),  # 2h d'overtime par jour
                overtime_hours=Decimal('2.00'),
                stars_earned=Decimal('0.25'),
                points_earned=20
            )
        ...
        self.assertEqual(performance.total_subtasks, 200)
        self.assertEqual(performance.total_overtime_hours, Decimal('40.00'))
        self.assertEqual(performance.monthly_stars, Decimal('5.00'))  # 20 * 0.25
        self.assertEqual(performance.bonus_stars, Decimal('0.50'))  # Bonus pour >32h overtime
        self.assertEqual(performance.total_stars, Decimal('5.50'))
...
class EmployeeBadgeModelTest(TestCase):
    ...
    def test_create_employee_badge(self):
        ...
        self.assertIsNotNone(employee_badge.earned_at)
