    ...
    def validate(self, data):
        ...
        try:
            ...
            if user and user.check_password(password):
                logger.info(
                    f"[JWTCreateWithEmailSerializer] Authentification réussie - Role: {user.role}"
                )
                data["user"] = user
                return data
            else:
                logger.warning(
                    f"[JWTCreateWithEmailSerializer] Échec d'authentification pour: {email}"
                )
                raise serializers.ValidationError("Email ou mot de passe incorrect.")

        except Exception as e:
            logger.error(
                f"[JWTCreateWithEmailSerializer] Erreur lors de l'authentification: {str(e)}"
            )
            raise serializers.ValidationError("Email ou mot de passe incorrect.")
...
class AdminLoginSerializer(JWTCreateWithEmailSerializer):
    ...
    def validate(self, data):
        ...
        if not user.is_admin:
            logger.warning(
                f"[AdminLoginSerializer] Tentative d'accès admin par non-admin: {user.email}"
            )
...
class EmployeeLoginSerializer(JWTCreateWithEmailSerializer):
    ...
    def validate(self, data):
        ...
        if not user.is_employee:
            logger.warning(
                f"[EmployeeLoginSerializer] Tentative d'accès employé par non-employé: {user.email}"
            )
...
class PasswordResetCodeVerifySerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        try:
            prc = PasswordResetCode.objects.filter(user=user, code=code, is_used=False).latest(
                "created_at"
            )
