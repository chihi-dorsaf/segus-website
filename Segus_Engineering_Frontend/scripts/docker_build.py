...
def main():
    parser = argparse.ArgumentParser(
        description="Script d'automatisation Docker pour le frontend Angular"
    )
    parser.add_argument(
        "--mode",
        choices=["dev", "prod"],
        default="dev",
        help="Mode de build (dev/prod)",
    )
    parser.add_argument(
        "--build", action="store_true", help="Construire l'image Docker"
    )
    parser.add_argument(
        "--push", action="store_true", help="Pousser l'image vers le registry"
    )
    parser.add_argument(
        "--clean", action="store_true", help="Nettoyer les images non utilisées"
    )
    ...
    parser.add_argument(
        "--compose", action="store_true", help="Utiliser docker-compose"
    )
    parser.add_argument(
        "--up", action="store_true", help="Démarrer les services avec docker-compose"
    )
    parser.add_argument(
        "--down", action="store_true", help="Arrêter les services docker-compose"
    )
    parser.add_argument(
        "--logs", action="store_true", help="Afficher les logs des conteneurs"
    )
    ...
    if args.compose:
        compose_file = (
            "docker-compose.dev.yml" if args.mode == "dev" else "docker-compose.yml"
        )
        ...
        if args.up:
            ...
            stdout, stderr, code = run_command(
                up_command, cwd=project_dir, capture_output=False
            )
            ...
            if code == 0:
                ...
                stdout, stderr, code = run_command(
                    f"docker-compose -f {compose_file} ps", cwd=project_dir
                )
                if code == 0:
        ...
        if args.down:
            ...
            stdout, stderr, code = run_command(
                down_command, cwd=project_dir, capture_output=False
            )
    ...
    if args.build and not args.compose:
        ...
        # Taille de l'image
        stdout, stderr, code = run_command(
            f"docker images {image_name} --format 'table {{{{.Repository}}}}\\t{{{{.Tag}}}}\\t{{{{.Size}}}}'"
        )
        if code == 0:
            print(f"\n📊 Tailles des images:")
            print(stdout)
