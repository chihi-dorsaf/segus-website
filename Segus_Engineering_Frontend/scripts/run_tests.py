...
def main():
    parser = argparse.ArgumentParser(
        description="Script d'automatisation pour les tests Angular"
    )
    parser.add_argument(
        "--coverage", action="store_true", help="Générer le rapport de couverture"
    )
    parser.add_argument(
        "--watch", action="store_true", help="Mode watch pour les tests"
    )
    parser.add_argument(
        "--headless", action="store_true", help="Exécuter en mode headless"
    )
    parser.add_argument(
        "--single-run", action="store_true", help="Exécution unique des tests"
    )
    parser.add_argument(
        "--lint", action="store_true", help="Exécuter le linting avant les tests"
    )
    parser.add_argument(
        "--build", action="store_true", help="Builder l'application avant les tests"
    )
    ...
    try:
        result = subprocess.run(test_command, shell=True, cwd=project_dir, check=False)
